...
export const mark_table = `
**GLOBAL EXTRACTION RULE**

Always look for a **Mark Table / Mark Schedule / Element Mark Table / Structural Mark List** in the drawing.

When a mark table is present, it is the **primary and authoritative source** for all mark-related information.

Always extract mark information from the **Mark Table** whenever the required value is available there.

The Mark Table takes priority over:
- Drawing annotations
...
1. Mark Table / Mark Schedule
2. Mark-specific drawing annotations
3. General arrangement or detail drawings
4. Inference from surrounding context

If a value is available in the Mark Table, **ALWAYS use the value from the table**, even if the same value appears elsewhere in the drawing.

**NEVER calculate, infer, estimate, or visually interpret a value when that value is explicitly provided in the Mark Table.**

For example:
- If the Mark Table gives mark = "CSM1", use "CSM1". Do not derive the mark from a label on the drawing.
- If the Mark Table gives description = "Column Starter Mesh Type 1", use that exact description.
- If the Mark Table gives qty = 4, use 4. Do not count elements visually from the drawing.

**FIELD-SPECIFIC RULES**

- mark:
  Extract the mark name/identifier from the corresponding **Mark / Mark Name / Element Mark / Reference / ID** column of the Mark Table.
  Marks typically follow naming conventions such as CSM1, CMS-2, GC-1, IP-1, BM-1, SB-1, etc.
  Preserve the exact mark string as written in the table, including any hyphens, numbers, or suffixes.
  Do not derive the mark from labels or annotations on the drawing unless it is not available in the table.
  Return null if the mark is not available.

- description:
  Extract the description from the corresponding **Description / Desc. / Element Description / Details** column of the Mark Table.
  The description typically specifies the type or purpose of the structural element (e.g. "Column Starter Mesh", "Ground Beam", "Isolated Pad Foundation", "Shear Wall Starter").
  Preserve the description text as written in the table.
  Do not paraphrase, abbreviate, or reconstruct the description from other parts of the drawing.
  Return null if the description is not available.

- qty:
  Extract the quantity from the corresponding **Qty / Quantity / No. / Nos. / Number / Count** column of the Mark Table.
  The quantity represents the number of times that particular mark/element occurs in the structure.
  Use the table value directly.
  Do not visually count elements from the drawing.
  Do not calculate the quantity if it is explicitly provided in the table.
  Return null if the quantity is not available.

**TABLE COLUMN VARIATIONS**

Column names may vary between drawings. Recognize equivalent headings such as:

- Mark → Mark Name, Element Mark, Reference, ID, Element ID, Part Mark
- Description → Desc., Element Description, Details, Remarks, Type
- Qty → Quantity, No., Nos., Number, Count, Repetition

**HANDLING MULTIPLE MARK TABLES**

Some drawings may contain more than one mark table (e.g. one for structural elements and one for reinforcement elements). In such cases:
- Extract marks from **all** mark tables present in the drawing.
- Preserve the source context of each mark if distinguishable.
- Do not merge or deduplicate marks across separate tables unless they clearly refer to the same item.

**FINAL RULE**

The **Mark Table is the source of truth for mark data**.

If the same information appears in both the Mark Table and the drawing, **always select the Mark Table value**.

Use the drawing or other sources only to obtain information that is missing from the Mark Table.
`;
